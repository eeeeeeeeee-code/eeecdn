...
func init() {
	ASNDB, err = geoip2.FromBytes(getlite2_asn)
	if err != nil {
		return
	}
...
func CheckCNAME(domain string) (bool, error) {
	var cname string
	if checkIP(domain) {
...
func CheckCIDR(ip string) (bool, string) {
	for _, cidr := range CDN_CIDR {
		...
		if ipnet.Contains(net.ParseIP(ip).To4()) {
			return true, cidr
		}
	}
...
func CheckASN(ip string) (bool, uint) {
	asn, err := ASNDB.ASN(net.ParseIP(ip).To4())

	if err != nil {
		fmt.Println("CheckASN " + err.Error())
		return false, 0
	}
	...
	if contains != -1 {
		...
		return true, asn.AutonomousSystemNumber
	}
	return false, 0
}

func CheckQqwr(ip string) (string, string) {
...
func CheckHttp(domain string) (string, error) {
	// 域名处理，可能传入是URL
	if strings.HasPrefix(domain, "http") {
		parse, err := url.Parse(domain)
		if err != nil {
			Timelog(domain+" URL解析错误", "red")
			return domain, err
		}
